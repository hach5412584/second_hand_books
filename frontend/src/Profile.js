...
import { Container, AppBar, Toolbar, Typography, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Box, Tab, Tabs } from '@mui/material';
    ...
    useEffect(() => {
        if (user && user.ID) {
            const fetchBooks = async () => {
                try {
                    const response = await fetch(`http://localhost:8080/api/booklist?userID=${user.ID}`);
                    if (!response.ok) {
            ...
            <Container>
                ...
                <Box mt={4}>
                    ...
                    {view === 'purchaseHistory' && (
                        <Typography variant="body1">歷史購買紀錄內容尚未實作。</Typography>
                    )}
                </Box>
